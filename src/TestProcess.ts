// DEBUGGER may hit invisible breakpoint on "use strict" line above
// ... just hit F5 to continue.
...
function createMocha() {
  ...
  if (args.options) {
    ...
    console.log(
      `No Mocha options are configured.\n` +
...
function resolveGlob(): Promise<string[]> {
  return new Promise((resolve, reject) => {
    ...
    new Glob(
      args.glob,
      { cwd: cmd, ignore: args.ignore, dot: true },
      (err, files) => {
        if (err) {
          return reject(err);
        }
